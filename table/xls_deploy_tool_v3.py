...
class DataParser:
    ...
    def __init__(self, xls_file_path, sheet_name):
        ...
        try :
            ...
            LOG_INFO("open xls file(%s) failed!", self._xls_file_path) 
            raise

        try :
            self._sheet =self._workbook.sheet_by_name(self._sheet_name)
        except BaseException, e :
            LOG_INFO("open sheet(%s) failed!", self._sheet_name) 
            raise
        ...
        try:
            ...
            LOG_INFO("load module(%s) failed", self._module_name) 
            raise
    ...
    def _WriteData2File(self, data) :
        file_name = self._sheet_name.lower() + ".data"
