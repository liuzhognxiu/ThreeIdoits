...
public class Driver : MonoBehaviour
{
    ...
    /// 资源列表名称
    /// </summary>
    private string filelist = "Android";
    /// <summary>
    /// 资源更新列表，更新完后要删除
    /// </summary>
    ...
    void Start()
    {
        ...
        ShowMessage = transform.Find("Info").GetComponent<Text>();
    ...
    public void CheckBundleList()
    {
        ...
        {
            if (Debug.developerConsoleVisible) Debug.LogError("start " + Time.realtimeSinceStartup);
            //开始解压资源
            StartCoroutine(ExportFileReources(() =>
            {
                ...
                StartCoroutine(CheckResources(() =>
                {
                    ...
                    {
                        if (Debug.developerConsoleVisible) Debug.LogError("end " + Time.realtimeSinceStartup);
                        //开始准备运行初始化
                        StartLoadData();
                    }));
    ...
    public Assembly LoadAssembly(string dllPath)
    {
        ...
        {
            if (Debug.developerConsoleVisible) Debug.Log("Demo.dll文件不存在！无法加载程序集！");
            return null;
        }
        else
        {
    ...
    IEnumerator CheckResources(Action UpdateData = null)
    {
        ...
        //}
        if (Debug.developerConsoleVisible) Debug.LogError("endtime == " + Time.realtimeSinceStartup);
        ...
        {
            if (Debug.developerConsoleVisible) Debug.Log("读取资源列表失败，不存在！");
            yield return null;
        }
        else
        {
            ...
            {
                if (Debug.developerConsoleVisible) Debug.Log("获取本地Manifest失败！");
            }
            else
            {
                //获取最新的Manifest文件
                ...
                {
                    if (Debug.developerConsoleVisible) Debug.Log("获取最新的Manifest失败！");
                }
                else
                {
    ...
    IEnumerator UpdateResource(Action beginRun = null)
    {
        ...
        if (isNeedUpdate)
        {
            ...
            if (Debug.developerConsoleVisible) Debug.Log(files.Length);
    ...
    IEnumerator DownloadResource(string bundleName)
    {
        ...
        using (WWW wwwfile = new WWW(inFilePath))
        {
            ...
            {
                if (Debug.developerConsoleVisible) Debug.Log("资源名不存在：" + wwwfile.error);
            }
            else
            {
    ...
    IEnumerator ExportFileReources(Action UpdateReoures = null)
    {
        ...
        using (WWW wwwManifest = new WWW(filePath))
        {
            ...
            {
                if (Debug.developerConsoleVisible) Debug.Log("获取AssetBundleManifest出错！");
            }
            else
            {
    ...
    public bool IsNeedExportAssetBundle()
    {
        ...
        if (Debug.developerConsoleVisible) Debug.Log(outManifestPath);
    ...
    public IEnumerator ExportDll()
    {
        ...
        using (WWW www = new WWW(inDllPath))
        {
            ...
            if (www.error != null)
            {
                if (Debug.developerConsoleVisible) Debug.Log("工程内部不存在dll文件！");
            }
            else
            {
